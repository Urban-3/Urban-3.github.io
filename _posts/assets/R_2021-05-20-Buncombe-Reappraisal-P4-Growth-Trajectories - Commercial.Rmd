...
# Introduction
For this phase of the project, we will explore historical appraisal data from 2001-2021. The purpose of analysis is to evaluate percent changes in both land and total taxable value across time and to compare growth trajectories between cities, neighborhoods, and individual parcels. The appraisers office maintains a historical appraisal record file (CAMA) with valuations dating back to 2001. Not all parcels contain complete records from 2001-2021. This is because new parcels are created from lot splits, consolidations, etc. Therefore, if the first record date is after 2001, this usually means that the parcel did not exist prior to this date and was created through one of the methods listed above. 

For the first phase of this analysis, we are primarily interested in parcels that have complete records from 2001-2021 and comparing the same parcel against itself across time. The following code was used to retrieve, clean, analyze and visualize the data. Since we are completing this project during the 2021 reappraisal period, we will be working with the most recently produced historical dataset (from 2019) and adding the 2021 reappraisal values. At the time of this analysis, not all 2021 reappraisal values were made publically available (in part due to ongoing appeals). This analysis should be re-run once the full 2021 data has been published.
...
We select a series of individual parcels to compare. For this phase, we've selected parcels that have already been featured in the developing presentation to include in the analysis. 
Create a list from the identified parcel identification numbers (PIN) to use to filter the historical appraisal data. We'll use the following four PINs as an example:

- 963828535800000 = Patton Ave KMart
- 965843867100000 = Bleachery Blvd WalMart

We'll be calculating percent change in two ways. The first way calculate the change in value from one year to the next. The second calculates the percent change from the 2001 baseline value.
...
##Preparation
parcels<-c("963828535800000", "965843867100000", "964930488300000") ##Create the list of parcels used to filter the full appraisal history dataset
...
ggplot(data=appraisal_history2019_21, aes(x=year, y=totalvalue, group=PIN)) +
  geom_line(aes(color=PIN))+
  xlab("Year") + ylab("Change in Total Taxable Value")+
  scale_color_discrete(name = "Individual Parcel", labels=c("Cecil","Powell","Lordman","Minicozzi"))+
  scale_y_continuous(labels=scales::dollar_format())
```

#### Figure 2: Percent Change in TOTAL Taxable Value from 2001 -- Individual Parcel Comparisons
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21, aes(x=year, y=baselinechange, group=PIN)) +
  geom_line(aes(color=PIN))+
  xlab("Year") + ylab("Percent Change in Total Taxable Value")+
  scale_color_discrete(name = "Individual Parcel", labels=c("Cecil","Powell","Lordman","Minicozzi"))
```

#### Figure 3: Percent Change in LAND Value from 2001 -- Individual Parcel Comparisons
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21, aes(x=year, y=landbaselinechange, group=PIN)) +
  geom_line(aes(color=PIN))+
  xlab("Year") + ylab("Percent Change in Land Value")+
  scale_color_discrete(name = "Individual Parcel", labels=c("Cecil","Powell","Lordman","Minicozzi"))
```

# Comparing growth trajectories between homes of different value

For this phase of the analysis, we group homes based on the total taxable value in 2001 into five equally sized groups (each representing 20% of the housing stock), from low value to high value. We then compare the average trajectories of those homes across time, to evaluate whether there are differences in how home values change according to initial home value. There are a few factors that could influence the appreciation in home values across all the value brackets. Homes at any value can be purchased, renovated, and resold at a higher price, thereby increase the valuation. There may be greater market demand for homes within a particular value bracket, which creates upward pressure on the home's value. Or the assessed value of any home could be artificially inflated due to systematic discrepancies in the assessment process. See the first phase of this project (J-curves) for a investigation into the systematic assessment bias of lower valued homes.
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_s, aes(x=year, y=meanval, group=Quintile)) +
  geom_line(aes(color=Quintile))+
  xlab("Year") + ylab("Avg. Change in Total Taxable Value")+
  scale_color_discrete(name = "Starting Value", labels=c("Lowest 20%","","","","Highest 20%"))+
  scale_y_continuous(labels=scales::dollar_format())
```

#### Figure 5: Percent change in TOTAL taxable value from 2001-- comparisons across home value
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_s, aes(x=year, y=mean, group=Quintile)) +
  geom_line(aes(color=Quintile))+
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_s, aes(x=year, y=meanland, group=Quintile)) +
  geom_line(aes(color=Quintile))+
...
Here, we add a geographic crosswalk, to help identify which parcels are associated with the various municipalities in Buncombe County. The crosswalk was developed outside of this project, using ArcMap. 
```{r, eval=TRUE, message=FALSE, warning=FALSE}
options(scipen=999)
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_wneigh_s, aes(x=year, y=meanval, group=City)) +
  geom_line(aes(color=City))+
  xlab("Year") + ylab("Avg. Change in Total Taxable Value")+
  scale_color_discrete(name = "City", labels=c("Asheville","Biltmore Forest","Black Mountain","Montreat", "Woodfin", "Weaverville"))+
  scale_y_continuous(labels=scales::dollar_format())
```

#### Figure 8: Percent change in TOTAL taxable value from 2001-- comparisons between municipalities 
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_wneigh_s, aes(x=year, y=mean, group=City)) +
  geom_line(aes(color=City))+
  xlab("Year") + ylab("Avg. Percent Change in Total Taxable Value")+
  scale_color_discrete(name = "City", labels=c("Asheville","Biltmore Forest","Black Mountain","Montreat", "Woodfin", "Weaverville"))
```
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_wneigh_s, aes(x=year, y=meanland, group=City)) +
  geom_line(aes(color=City))+
  xlab("Year") + ylab("Avg. Percent Change in Land Value")+
  scale_color_discrete(name = "City", labels=c("Asheville","Biltmore Forest","Black Mountain","Montreat", "Woodfin", "Weaverville"))
```
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_wneigh_s, aes(x=year, y=meanval, group=name)) +
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_wneigh_s, aes(x=year, y=mean, group=name)) +
  geom_line(aes(color=name))+
  xlab("Year") + ylab("Avg. Percent Change in Total Taxable Value")+
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
ggplot(data=appraisal_history2019_21_full_wneigh_s, aes(x=year, y=meanland, group=name)) +
  geom_line(aes(color=name))+
  xlab("Year") + ylab("Avg. Percent Change in Land Value")+
  scale_color_discrete(name = "City")
```